...
class BertEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.token_type_embeddings = nn.Embedding(2, config.emb_size) # 用于区分是token属于哪个句子，用于句子级别的预测
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, emb, att_mask):
        ...
        if att_mask != None: # [batch size, seqlen]
            att_mask = (att_mask > 0).unsqueeze(1).repeat(1, att_mask.size(1), 1).unsqueeze(1) # [batch_size, 1, seqlen]->[batch_size,seqlen,seqlen]->[batch_size,1,seqlen,seqlen]
...
class BertForSequenceClassification(nn.Module):
    ...
    def reduce_seq_len(self, seq_len):
        ...
        self.bert.embeddings.position_ids = self.bert.embeddings.position_ids[:, :seq_len] # (1, max_seq_length)
